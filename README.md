...
[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-Built%20with%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=Nordlet%2FGo)

The Nordlet Go library provides convenient access to the Nordlet APIs from Go.
...
```go
client := client.NewClient(
    option.WithBaseURL(nordlet.Environments.Production),
)
```
...
```go
response, err := client.Reference.PostV1ReferenceExchangeRatesSync(...)
if err != nil {
...
```go
response, err := client.Reference.WithRawResponse.PostV1ReferenceExchangeRatesSync(...)
if err != nil {
    return err
}
...
response, err := client.Reference.PostV1ReferenceExchangeRatesSync(ctx, ...)
```
...
response, err := client.Reference.PostV1ReferenceExchangeRatesSync(ctx, request, ...)
```
